            ...
            if method == "tools/list":
                result = {
                    "tools": [
                        ...
                        {"name": "assign_verifier", "description": "Assign a verifier for a job."},
                        ...
                        {"name": "get_reputation", "description": "Get reputation data for a profile id."},
                        {"name": "get_balance", "description": "Get wallet balances and locked funds."},
                        {"name": "register_profile", "description": "Register profile if not already registered."},
                        {"name": "claim_expired", "description": "Claim a job after expiry."},
                        {"name": "update_axl_key", "description": "Update profile AXL key on-chain."},
                    ]
                }
                ...
                if name == "bid_job":
                ...
                elif name == "assign_verifier":
                    result = assign_verifier(args, self.axl_client, self.contract_client)
                    ...
                    result = get_output(args, self.contract_client, self.storage, caller_id)
                ...
                elif name == "register_profile":
                    result = register_profile(args, self.contract_client)

                elif name == "claim_expired":
                    result = claim_expired(args, self.contract_client)

                elif name == "update_axl_key":
                    result = update_axl_key(args, self.contract_client)
